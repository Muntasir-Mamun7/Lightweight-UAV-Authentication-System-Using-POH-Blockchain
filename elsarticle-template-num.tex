...
\begin{enumerate}
    \item f{Lightweight PoH Engine:} We implemented a sequential hashing engine that generates a tamper-evident timeline of UAV telemetry, optimized for standard flight controllers.
...
Several promising research directions emerge from this foundation. Extending the single-node PoH architecture to a decentralized network with multiple Ground Control Stations could enhance resilience through cross-validation, potentially using Directed Acyclic Graph (DAG) structures. The immutable PoH ledger provides trustworthy datasets for AI-driven anomaly detection, enabling predictive security alerts for GPS spoofing or unauthorized trajectory deviations. Migrating to post-quantum cryptographic primitives (e.g., SPHINCS+) would future-proof against emerging quantum threats. The PoH authentication model also generalizes beyond UAVs to autonomous vehicles, industrial IoT, and medical devices. Finally, collaborating with aviation authorities (FAA, EASA, ICAO) for pilot studies would accelerate regulatory acceptance and industry adoption.
